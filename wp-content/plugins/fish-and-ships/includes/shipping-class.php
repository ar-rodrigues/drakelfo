...
class WC_Fish_n_Ships extends WC_Shipping_Method {
	...
	public function process_admin_options(){
		...
		// Reset messages and coupons on session.
	...
	*/
	...
	public function calculate_shipping( $package = array() ) {
		...
		foreach ($this->shipping_rules as $shipping_rule) {
				...
				if (isset($shipping_rule['sel'])) {
					foreach ($shipping_rule['sel'] as $n_key=>$selector) {
						...
						if (is_array($selector) && isset($selector['method'])) {
							...
							$rule_groups = apply_filters('wc_fns_check_matching_selection_method', $rule_groups, $selector, $group_by, $this, $logical_operator);
			...
			$this->debug_log('*- Calculated rule #' . ($rule + 1) . ' cost: ' . $Fish_n_Ships->unabstracted_price( $rule_cost ), 1);
		...
		if ( $this->free_shipping == 'no' && $global_cost == 0 ) {
			$active = false;
		...
		if ($active) {
			...
			if (trim($this->min_shipping_price) != '') {
				...
				if ( $min_shipping_price > $global_cost ) {
					$global_cost = $min_shipping_price;
			...
			if ( trim($this->max_shipping_price) != '' && $this->max_shipping_price != 0 ) {
				...
				if ( $max_shipping_price < $global_cost) {
					$global_cost = $max_shipping_price;
