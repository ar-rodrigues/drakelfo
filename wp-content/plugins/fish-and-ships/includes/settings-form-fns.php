...
function wc_fns_get_selection_methods_fn($methods = array()) {
	...
	$methods['lgirth-dimensions']  = array('onlypro' => true,  'label' => _x('Length+Girth (L+2W+2H)', 'shorted, select-by conditional', 'fish-and-ships'));
	...
	$methods['cart-total']         = array('onlypro' => true,  'label' => _x('Cart total', 'shorted, select-by conditional', 'fish-and-ships'));
	$methods['n-groups']           = array('onlypro' => false, 'label' => _x('Number of groups', 'shorted, select-by conditional', 'fish-and-ships'));

	$methods['user-role']          = array('onlypro' => true,  'label' => _x('User role', 'shorted, select-by conditional', 'fish-and-ships'));

	$methods['always']             = array('onlypro' => false, 'label' => _x('Always (all match)', 'shorted, select-by conditional', 'fish-and-ships'));
...
function wc_fns_sanitize_selection_fields_fn($rule_sel) {
	...
	switch ($rule_sel['method']) {
		...
		case 'cart-total':
			...
			if ($rule_sel['method'] == 'cart-total') unset( $allowed[2] );
	...
	if (is_array($allowed)) {
		...
		switch ($rule_sel['method']) {
			...
			case 'quantity':
				...
				// by-price & cart-total has one min and max fields for every currency
				if ( $rule_sel['method'] == 'by-price' || $rule_sel['method'] == 'cart-total' ) {
					$n = 0;
					foreach ( $currencies as $currency => $symbol ) {
...
function wc_fns_check_matching_selection_method_fn($rule_groups, $selector, $group_by, $shipping_class, $logical_operator = 'and') {
	...
	switch ($selector['method']) {
		...
		case 'quantity':
			...
			if ( $selector['method']=='by-price' ||  $selector['method']=='cart-total' ) {
...
function wc_fns_get_html_price_fields_fn($html, $rule_nr, $values) {
	...
	*/
	...
	foreach ( $currencies as $currency=>$symbol ) {
		...
		$html .= '<input type="text" name="shipping_rules[' . $rule_nr . '][cost][cost'.$curr_sufix.'][]" value="' . esc_attr( $value ) . '" placeholder="0" size="4" class="wc_fns_input_decimal fns-cost" autocomplete="off">';
		$html .= '</span>';
	}
	$html .= '</span></span>';

	/*
	<input type="text" name="shipping_rules[' . $rule_nr . '][cost][cost][]" value="' . esc_attr( $cost ) . '" placeholder="0" size="4" class="wc_fns_input_decimal fns-cost" autocomplete="off"></span>*/
	...
	foreach ( $loop_data as $field_name => $label) {
		...
		foreach ( $currencies as $currency=>$symbol ) {
			...
			$html .= '<span class="currency-fns-field currency-' . $currency . ($n==1 ? ' currency-main' : '') . '">';
			$html .= '<input type="text" name="shipping_rules[' . $rule_nr . '][cost][' . $field_name . $curr_sufix . '][]" value="' . esc_attr( $value ) . '" placeholder="0" size="4" class="wc_fns_input_decimal fns-'.$field_name.'" autocomplete="off"> ' . esc_html( ($field_name == 'cost_percent' ? '' : $symbol . ' ') . $label );
			$html .= '</span>';
		}
		$html .= '</span></span>';
	}
...
function wc_fns_get_actions_fn($actions = array()) {
	...
	*/
	...
	$actions['break']           = array('onlypro' => false, 'label' => _x('Stop (ignore below rules)', 'shorted, action name (renamed on v1.2.1)', 'fish-and-ships'));
	$actions['min_max']         = array('onlypro' => true,  'label' => _x('Set min/max rule costs', 'shorted, action name', 'fish-and-ships'));
	$actions['unset']           = array('onlypro' => true,  'label' => _x('Matching prods skip below rules', 'shorted, action name (renamed on v1.2.1)', 'fish-and-ships'));
	...
	$actions['coupon']          = array('onlypro' => true,  'label' => _x('Auto-apply coupon', 'shorted, action name', 'fish-and-ships'));
	$actions['math']            = array('onlypro' => true,  'label' => _x('Math expression', 'shorted, action name', 'fish-and-ships'));
	$actions['boxes']           = array('onlypro' => true,  'label' => _x('Shipping boxes', 'shorted, action name', 'fish-and-ships'));
	$actions['disable_others']  = array('onlypro' => true,  'label' => _x('Hide other shipping methods', 'shorted, action name', 'fish-and-ships'));
