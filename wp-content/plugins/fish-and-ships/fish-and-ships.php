...
if ( defined('WC_FNS_VERSION') || class_exists( 'Fish_n_Ships' ) ) {
	...
	class Fish_n_Ships {
		 ...
		 *
		 * WOOCS AND WMC_F works in main currency and does a currency conversion if needed at the end itself
		...
		function currency_abstraction ($currency_origin, $value ) {
			...
			if ( $this->is_wpml_mc  && $currency_origin == 'main-currency' ) {
				...
				$value = apply_filters( 'wcml_raw_price_amount', $value, get_woocommerce_currency() );
			}
		...
		if ( $this->is_woo_mc || $this->is_aelia_mc || $this->is_wpml_mc || $this->is_alg_cs ) return true;
		...
		public function sanitize_number($number, $type = 'unknown') {
			...
			// PHP8 fatal error solved, preventing empty text value ''
			$number = (float) $number;
		 ...
		 *
		 */
		function get_decimal_field ($field_name, $rule_nr, $sel_nr, $method_id, $lang_sufix, $units, $values, $ambit_field, $ambit, $tips, $comp) {
			...
			$html = '<input type="text" name="shipping_rules[' . $rule_nr . ']['.$ambit_field.']['.$method_id.']['.$field_name.$lang_sufix.']['.$sel_nr.']" size="4" value="' . esc_attr($value) . '" data-wc-fns-tip="i18n_'.$field_name.'_' . $tips . esc_attr($comp) . '"'
					. ' class="wc_fns_input_decimal wc_fns_input_tip" placeholder="0" autocomplete="off">';
